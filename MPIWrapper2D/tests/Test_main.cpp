...
int main(int argc, char** argv) {
	...
	*/
	...
	auto tab = DataTable();
	std::vector<std::string> columns = {"a1", "b2", "c3", "d4"};
	tab.initialize(columns);
	tab.append_value("a1", 1.0);
	tab.append_value("b2", 1.0);
	tab.append_value("c3", 1.0);
	tab.append_value("d4", 2.0);
	tab.append_value("d4", 3.0);
	tab.save_table("test.txt");
